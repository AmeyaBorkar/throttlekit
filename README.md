...
- **A formally-verified overshoot bound — independent of fleet size *and* region count.** The two-tier leasing path is model-checked in TLA⁺/TLC: worst-case global admissions are *exactly* `Limit + N·(Batch−1)` (shown tight by counterexample), and with `windowCoupled` they collapse to *exactly* `Limit` — **no matter how many nodes**. **0.8.3 lifts this to cross-cluster federation** (`federate(...)` against a `RedisCoordinator`): `admitted ≤ Limit` **independent of the region count K** too. Most limiters can't state a bound at all; this one is machine-checked at every layer, and the checker re-runs in CI.
...
- **`unifiedAdmission`** — compose rate ∧ concurrency ∧ cost into one decision (AND / MIN / MAX algebra), sequential or atomic Lua-fused. Opt into **`policy: "joint-lp"`** for a revenue-management **bid-price** filter on top — admit iff `value ≥ p_R + p_C·cost` (shadow prices from the workload's fluid LP) — steering a cost-bound budget to the highest-value requests (closes a measured **ε = 25.33%** revenue gap vs marginal-AND; opt-in, default unchanged). Add **`jointLp.adaptive`** to learn the bid prices online (sample-then-price) — it adopts the re-solved duals only if they beat the prior on the observed sample, rescuing a misspecified prior while never undercutting a good one on-sample. Or add a **concurrency** shadow price (3-axis: `value ≥ p_R + p_C·cost + p_K·hold`, via a `concBudget` + per-request `hold`) to reject a **hold-time hog** — a slow request that hogs a worker slot — that the cost axis is blind to.
...
Dual-path conformance (JS ≡ Lua on thousands of generated timelines, now including `quota`'s civil-calendar Lua and `peek` decoding the Redis-side state) + a **shrinkable** fast-check property pass over the consuming `check` path of all 6 Lua-backed strategies (TK-826), atomicity (N concurrent checks at limit K ⇒ exactly K allowed), the TLA⁺/TLC formal model re-checked by an exhaustive JS checker in CI, federation BFS-twin coverage including a `RedisCoordinator ≡ PostgresCoordinator` dual-path (TK-1302), multi-process regional escrow + regional-only outage mode wiring (TK-1306), **0.9.0's unified admission**: the four-law algebra (identity, associativity, commutativity, idempotency at numRuns ≥ 500 via fast-check), sequential ≡ Lua-fused byte-identical Decision streams across 300 timelines × 3 binding regimes (TK-1006), and an informational `bench:gate` regression gate (TK-827). All time-dependent tests use `ManualClock`, so the suite is deterministic. Current state: **1329 tests** (with the Redis/Postgres-gated suites enabled), CI green across Node 20/22/24. **0.9.1 adds Pillar 4 Weighted Fair Escrow** — a work-conserving, weight-honoring budget splitter across tenants with the four GALE fairness theorems property-checked at `numRuns: 200` plus L1 ≡ L2 dual-path conformance. **Federated WFE (TK-1404) lifts Pillar 4 across regions** — `federatedWeightedFairEscrow` + `regionFairPool` compose per-region tenant fairness into a *global* weighted-max-min guarantee (region weight = Σ active tenant weights, the Parekh–Gallager GPS collapse), with a machine-checked composition gate (fluid exactness on 4 + 400 worlds, a four-cell failure boundary, and shipped-code validation against the flat oracle) and `Σ admitted ≤ L` proven across regions at 854K randomized trials.
