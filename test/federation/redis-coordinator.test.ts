...
d("RedisCoordinator + federate (TK-906)", () => {
  ...
  describe("lease() / reconcile() — atomic Lua semantics", () => {
    ...
    it("leaseWindowed returns the AUTHORITATIVE window boundary + the same partial-grant semantics (FLA-1)", async () => {
      ...
      // The boundary is the Redis-TIME-derived window end — a multiple of windowMs, near now (NOT a
      // node-supplied value; lease() ignores its expiresAt arg, leaseWindowed reports the real one).
