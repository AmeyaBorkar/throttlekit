...
| Test coverage on `src` | ✅ **913 tests** (836 pass + 77 skipped without `THROTTLEKIT_TEST_REDIS`/`PG`; the 56-test growth in 0.8.5 is multi-process regional escrow + regional-only outage mode — TK-1306: 28 + 10 + 10 always-on across `regional-escrow.test.ts` / `regional-escrow-engine.test.ts` / `regional-only.test.ts`, plus 13 gated Redis tests in `redis-regional-escrow.test.ts`. All pass with the gated suites enabled. 0.8.0's 95.2% line coverage figure is from a coverage run on the previous test set, re-measure on release) |
