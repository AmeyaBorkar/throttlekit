...
| Redis `strict` decision | exactly **1** round trip | confirmed (single EVALSHA) | 🟡 |
...
| JS ↔ Lua dual-path bit-identical decisions | conformance vector suite | ✅ (gcra, token bucket, fixed window; ~49k round trips) |
| `remaining` never negative; never allow above limit+overshoot | property tests | 🟡 (enforced in code; property suite pending) |
| Leased overshoot ≤ L×B | property test | ⬜ |
| Clock-jump safety (negative elapsed clamped) | unit tests | ✅ (gcra) |
...
| MemoryStore (timer wheel + approx-LRU) | 🟡 (timer wheel ✅; CLOCK LRU pending) |
...
| Test coverage ≥ 90% lines on `src` | ⬜ (measured near release) |
| CI green (lint, typecheck, test, build, Redis matrix) | 🟡 (pushed; awaiting first run) |
| Build emits valid ESM + CJS + types | ✅ (index + redis entries) |
