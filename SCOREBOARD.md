...
| Test coverage on `src` | ✅ **1139 tests** (1027 pass + 112 skipped without `THROTTLEKIT_TEST_REDIS`/`PG`; the 56-test growth in 0.10.0 is the distributed-adaptive-concurrency suite — the heartbeat-leasing BFS twin (budget-capped `GlobalCap` model, distinct=64) + property invariant with simulated out-of-order grant landing + dual-path Test≡Redis coordinator conformance + unit coverage of the `distributedAdaptiveConcurrency` primitive (TK-1316), plus the new `adaptiveConcurrency`/distributed acquire-path bench. All pre-existing tests carried forward; bench gate green at the 0.9.0 baseline. 0.8.0's 95.2% line coverage figure is from a coverage run on the previous test set, re-measure on release) |
