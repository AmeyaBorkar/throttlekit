...
| Test coverage on `src` | ✅ **857 tests** (793 pass + 64 skipped without `THROTTLEKIT_TEST_REDIS`/`PG`; the 12-test growth in 0.8.4 is `PostgresCoordinator` conformance — all pass with the gated suites enabled. 0.8.0's 95.2% line coverage figure is from a coverage run on the previous test set, re-measure on release) |
