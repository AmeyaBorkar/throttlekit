...
| Admission Policy Plans (`throttlekit/policy`) — "terraform plan for limits": replay recorded traffic vs a candidate policy → exact allow↔deny diff (leaf rate/cost exact; every other axis `not-replayable`) | ✅ |
| What-If Replay (`throttlekit/testkit`) — record a leaf limiter's decisions → deterministic trace → replay a single-field candidate; multi-candidate `scorecard` | ✅ |
| Tier-1 distributed over existing RPCs (zero client change) — `federated:` / `federatedFairEscrow:` over Check, `fleetBudget:` over Debit, `distributedConcurrency:` over Admit | ✅ |
| Tier-2 Fleet lease (`throttlekit/twotier` `LeaseSpender` + server `Fleet.Reserve`) — lease a chunk of a `federated:` policy's global budget, spend locally byte-identical to the leased-L1 path, round-trip only to refresh | ✅ |
| Monitor door — read-only `throttlekit.v1.Monitor` gRPC (`GetSnapshot` + `Watch`) + Prometheus `/metrics` + gRPC health (the programmable/remote read API) | ✅ |
| IETF (draft + structured) + legacy headers, `Retry-After` | ✅ |
| Proxy-correct IP + IPv6 /64 aggregation + HMAC keys | ✅ |
| Store conformance testkit | ✅ |
| TypeScript-first, ESM + CJS, 24 entry points (+ a `throttlekit` CLI) | ✅ |
...
| Pillar 4 federated — hierarchical WFE across regions composes to **global** weighted-max-min (TK-1404) | composition theorem T-FED-1/2/3 machine-checked (fluid exactness = 0 dev on 4 + 400 worlds; F1–F4 failure boundary; shipped-code Part-5 validation vs flat oracle); Σ≤L proven across regions at 854K trials; **shipped** as `federatedWeightedFairEscrow` + `regionFairPool` | ✅ |
...
| Build emits valid ESM + CJS + types (24 subpaths — added `throttlekit/policy`) | ✅ |
