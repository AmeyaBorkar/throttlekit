...
- **Open (the honest roadmap):** (1) partial coordination is now characterised — the static-partition
  `0<C<N` interpolation and the dynamic `≤C`-message bound `Δ+N·U ≥ (N−1)(L−C·B)` (tight at `B=1`), both
  machine-checked; only the tight closed form for *batched* dynamic leasing (`B>1, C≥2`, where stranding
  opens a gap) remains open; (2) a distributed eval at scale on real LLM-serving traces; (3) the
  conjectured cost-axis trilemma.
