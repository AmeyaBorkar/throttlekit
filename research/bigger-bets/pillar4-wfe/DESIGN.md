...
| **DR-P4-2** | ~~Quantum semantics = **`quantum` is the per-tenant ceiling-raise unit**~~. *Why changed (TK-1310 implementation):* At the L1 layer the streaming algorithm recomputes `gᵢ = ⌊wᵢ/W·L⌋` exactly per check (`O(N)`), so there is no DRR quantum — T4 collapses to 0 slack at this layer. The `quantum` knob is **dropped from the L1 API** and reserved for the L2 path (where it controls the per-process lease size to the shared `Store` — the cross-process T4 bound). The 0.9.1 L1 API has no `quantum` option; the L2 commit (still inside TK-1310) reintroduces it scoped to the L2 lease. | Updated 2026-05-29 |
